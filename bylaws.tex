...
\subsection{Voting}
\begin{enumerate}
\item All issues requiring a vote, except when otherwise specified as a \textbf{Vote of the Membership} in these Bylaws or required by law, shall be decided by affirmative vote of \textbf{more than one half (50\%)} of the Members present at any Meeting.
\item Any Member may abstain from any Vote.  In this case, that Member shall count towards Quorum on an issue, but shall not count towards determining the minimum number of Votes needed to pass that issue.
\end{enumerate}
\subsection{Votes of the Membership}
\begin{enumerate}
\item Quorum for a \textbf{Vote of the Membership} shall require attendance of \textbf{at least half (50\%)} of the existing Membership on the day of the vote.
\item For the purposes of calculating the quorum, properly submitted proxy statements as well as votes submitted as described in the section \textbf{Remote Voting} shall count as attendance.
\item All \textbf{Votes of the Membership} shall require an affirmative vote of \textbf{more than half (50\%)} of the Members voting, unless a higher threshold is specified for a particular vote in the Bylaws.
\end{enumerate}
\subsection{Remote Voting}
A poll may be brought up online to handle collection of votes as long as the online voting system authenticates users as Members. Once the poll has been announced to the Membership, there will be a period of no less than \textbf{seven (7) days} to vote on the topic. After the deadline, the online poll is locked so no further votes are possible and the results are included with other voting methods described within the Bylaws.
